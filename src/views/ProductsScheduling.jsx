...
function EditProductModal({ product, onClose, onSaved }) {
  ...
  const addSubreddit = (e) => {
    if (e.key === 'Enter' && subInput.trim()) {
      e.preventDefault()
      if (!subreddits.includes(subInput.trim())) setSubreddits([...subreddits, subInput.trim()])
      setSubInput('')
    }
  }
  ...
  const handleSave = async () => {
    if (!name.trim()) return
    setSaving(true)
    await supabase.from('products').update({ name: name.trim(), category, keywords, target_subreddits: subreddits, active }).eq('id', product.id)
    setSaving(false)
    onSaved()
  }
    ...
    <tr>
      <td colSpan={8} className="p-0">
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-6 w-full max-w-xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-3">Edit Product</h2>

            {/* Tabs */}
            <div className="flex gap-1 mb-4 bg-gray-100 dark:bg-gray-800 rounded-lg p-1 w-fit">
              {[{ id: 'details', label: 'Details' }, { id: 'hashtags', label: 'Hashtags' }].map(t => (
                <button key={t.id} onClick={() => setModalTab(t.id)} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${modalTab === t.id ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm' : 'text-gray-600 dark:text-gray-400'}`}>{t.label}</button>
              ))}
            </div>
            ...
            {modalTab === 'details' && (
              <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {keywords.map((k, i) => (
                      <span key={i} className="px-2 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-xs rounded-full flex items-center gap-1">
                        {k}<button type="button" onClick={() => setKeywords(keywords.filter((_, j) => j !== i))} className="hover:text-red-500 text-sm leading-none">&times;</button>
                      </span>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {subreddits.map((s, i) => (
                      <span key={i} className="px-2 py-1 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 text-xs rounded-full flex items-center gap-1">
                        r/{s}<button type="button" onClick={() => setSubreddits(subreddits.filter((_, j) => j !== i))} className="hover:text-red-500 text-sm leading-none">&times;</button>
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button onClick={handleSave} disabled={saving || !name.trim()} className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50">{saving ? 'Saving...' : 'Save Changes'}</button>
                </div>
              </div>
            )}

            {modalTab === 'hashtags' && <HashtagsTab productId={product.id} />}
          </div>
        </div>
      </td>
    </tr>
...
function HashtagsTab({ productId }) {
  ...
  const PLATFORM_COLORS = {
    ...
    instagram: 'bg-fuchsia-100 dark:bg-fuchsia-900/30 text-fuchsia-700 dark:text-fuchsia-300',
    reddit: 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300',
  }

  const load = useCallback(async () => {
    const { data } = await supabase.from('product_hashtags')
      .select('*').eq('product_id', productId).order('platform').order('priority')
    setHashtags(data || [])
    setLoading(false)
  }, [productId])
  ...
  const addHashtag = async () => {
    ...
    const tag = newHashtag.trim().replace(/^#/, '').replace(/^r\//, '')
    await supabase.from('product_hashtags').insert({
      product_id: productId, platform: newPlatform,
      hashtag: tag, priority: newPriority, active: true,
    })
    setNewHashtag('')
    load()
  }
  ...
  const toggleActive = async (id, current) => {
    await supabase.from('product_hashtags').update({ active: !current }).eq('id', id)
    load()
  }

  const updatePriority = async (id, val) => {
    const p = Math.max(1, Math.min(10, parseInt(val) || 5))
    await supabase.from('product_hashtags').update({ priority: p }).eq('id', id)
    load()
  }

  const deleteHashtag = async (id) => {
    await supabase.from('product_hashtags').delete().eq('id', id)
    load()
  }
  ...
  const grouped = {}
  for (const p of PLATFORMS) grouped[p] = hashtags.filter(h => h.platform === p)

  return (
    <div className="space-y-4">
      <p className="text-xs text-gray-500 dark:text-gray-400">
        Hashtags are loaded by agents during scraping. Higher priority (lower number) hashtags are searched first.
        For Reddit, enter subreddit names (without r/).
      </p>

      {/* Add new */}
      <div className="flex items-end gap-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
        <div>
          <label className="block text-[10px] font-medium text-gray-500 mb-1">Platform</label>
          <select value={newPlatform} onChange={e => setNewPlatform(e.target.value)} className="text-xs border border-gray-300 dark:border-gray-600 rounded px-2 py-1.5 bg-white dark:bg-gray-900 text-gray-900 dark:text-white">
            {PLATFORMS.map(p => <option key={p} value={p}>{p}</option>)}
          </select>
        </div>
        <div className="flex-1">
          <label className="block text-[10px] font-medium text-gray-500 mb-1">{newPlatform === 'reddit' ? 'Subreddit' : 'Hashtag'}</label>
          <input type="text" value={newHashtag} onChange={e => setNewHashtag(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && addHashtag()}
            className="w-full text-xs border border-gray-300 dark:border-gray-600 rounded px-2 py-1.5 bg-white dark:bg-gray-900 text-gray-900 dark:text-white"
            placeholder={newPlatform === 'reddit' ? 'SkincareAddiction' : 'sheetmask'} />
        </div>
        <div>
          <label className="block text-[10px] font-medium text-gray-500 mb-1">Priority</label>
          <input type="number" min={1} max={10} value={newPriority} onChange={e => setNewPriority(parseInt(e.target.value) || 3)}
            className="w-14 text-xs border border-gray-300 dark:border-gray-600 rounded px-2 py-1.5 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-center" />
        </div>
        <button onClick={addHashtag} className="px-3 py-1.5 bg-indigo-600 text-white rounded text-xs font-medium hover:bg-indigo-700">Add</button>
      </div>
      ...
      {PLATFORMS.map(platform => (
        <div key={platform}>
          <div className="flex items-center gap-2 mb-2">
            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${PLATFORM_COLORS[platform]}`}>{platform}</span>
            <span className="text-[10px] text-gray-400">{grouped[platform].length} hashtags</span>
          </div>
          {grouped[platform].length === 0 ? (
            <p className="text-xs text-gray-400 ml-2 mb-2">No hashtags configured for this platform.</p>
          ) : (
            <div className="space-y-1 mb-3">
              {grouped[platform].map(h => (
                <div key={h.id} className={`flex items-center gap-2 px-2 py-1.5 rounded text-xs ${h.active ? 'bg-white dark:bg-gray-900' : 'bg-gray-100 dark:bg-gray-800 opacity-60'} border border-gray-200 dark:border-gray-700`}>
                  <button onClick={() => toggleActive(h.id, h.active)}
                    className={`w-7 h-4 rounded-full transition-colors relative shrink-0 ${h.active ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-gray-600'}`}>
                    <span className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-transform ${h.active ? 'left-3.5' : 'left-0.5'}`} />
                  </button>
                  <span className="flex-1 font-medium text-gray-900 dark:text-white">{platform === 'reddit' ? `r/${h.hashtag}` : `#${h.hashtag}`}</span>
                  <input type="number" min={1} max={10} value={h.priority} onChange={e => updatePriority(h.id, e.target.value)}
                    className="w-10 text-center text-[10px] border border-gray-200 dark:border-gray-700 rounded bg-transparent text-gray-600 dark:text-gray-400" />
                  <span className="text-[10px] text-gray-400 w-4">P{h.priority}</span>
                  <button onClick={() => deleteHashtag(h.id)} className="text-red-400 hover:text-red-600 text-sm leading-none">&times;</button>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="flex gap-3 pt-2">
        <button onClick={() => {}} className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800" disabled>Close (use X above)</button>
      </div>
    </div>
  )
}
